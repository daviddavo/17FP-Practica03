...
carta::tCarta tecla2carta(tecla::tTecla tecla) {
    switch (tecla) {
    case tecla::AVANZA: return carta::AVANZAR;
...
bool ejecutarSecuencia(tJuego & juego, tMazo & secuencia) {
    ...
    while (sacar(secuencia, paso) && !joya) {
        insertar(juego.jugadores[juego.turno].mazo, paso);
        juego.jugadores[juego.turno].mano[paso]--;
        switch (paso) {
        case carta::AVANZAR: joya = avanzar(juego); break;
...
bool ejecutarTurno(tJuego & juego) {
    ...
    addMsg(juego.log, juego.jugadores[juego.turno].nombre + ", juegas o robas? (R/E)");
    mostrarJuego(juego);
    do {
        if (MENU_GETCH) {
            c = _getch();
    ...
    if (manoVacia(juego.jugadores[juego.turno].mano)) {
        ...
        if (pedirSecuencia(juego, secuencia)) {
            ...
            juego.log[0] = juego.jugadores[juego.turno].nombre + " ha intentado una secuencia invalida";
...
void mostrarPuntuaciones(const tPuntuaciones &puntuaciones) {
  ...
  for (unsigned i = 0; i < puntuaciones.cnt; i++) {
    if (puntuaciones.puntuaciones[i].nombre.length() > maxw)
      maxw = puntuaciones.puntuaciones[i].nombre.length();
  }
  ...
  colorReset();
  for (unsigned i = 0; i < puntuaciones.cnt; i++) {
    cout << "\t\t\t" << std::setw(maxw + 2)
         << puntuaciones.puntuaciones[i].nombre << std::right << std::setw(5)

    ...
    for (int x = 0; x < MAX_FILAS && id < juego.nJugadores; x++) {
      for (int y = 0; y < MAX_FILAS && id < juego.nJugadores; y++) {
        if (juego.tablero[x][y].estado == TORTUGA) {
          ...
          tCoord coord = {x, y};
          juego.jugadores[id].pos = coord;
          juego.jugadores[id].spawn = coord;
...
bool avanzar(tJuego & juego) {
    ...
    if (calcularPos(tmpCoor, tortuga.direccion)) {
        ...
        case JOYA:
            ...
            /* no break */
        case CAJA:
            if (!empujar(juego, tmpCoor, tortuga.direccion))
...
void girar(tJuego & juego, const bool derecha) {
    tDir & direccion = juego.tablero[juego.jugadores[juego.turno].pos.x][juego.jugadores[juego.turno].pos.y].tortuga.direccion;
...
bool disparar(tJuego & juego) {
    ...
    case HIELO:
        juego.tablero[coord.x][coord.y].estado = VACIA;
        addMsg(juego.log, jugador.nombre + " ha destruido " + std::to_string(coord.x) + ", " +
